...
export default function PersonalizedAgentPanel() {
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError("推荐失败，请检查后端服务、AMap Key 和 LLM 配置后重试。");
      setResult(null);
    } finally {
      setLoading(false);
    }
    ...
    <section className="rounded-[24px] border border-[#ead8c2] bg-[#fff8ea]/88 p-5 shadow-[0_18px_56px_rgba(104,72,42,0.10)] backdrop-blur">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-[#806653]">规则打分 + 通勤评估 + LLM 重排（可选）</p>
        </div>
      </div>
      ...
      <form className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4" onSubmit={onSubmit}>
        ...
        <Field label="医疗权重">
          <input
            ...
            onChange={(event) => setForm((prev) => ({ ...prev, medical_weight: Number(event.target.value) || 0 }))}
          />
        </Field>
        ...
        <Field label="教育权重">
          <input
            ...
            onChange={(event) => setForm((prev) => ({ ...prev, education_weight: Number(event.target.value) || 0 }))}
          />
        </Field>
        ...
        <Field label="休闲权重">
          <input
            ...
            onChange={(event) => setForm((prev) => ({ ...prev, recreation_weight: Number(event.target.value) || 0 }))}
          />
        </Field>

        <Field label="Top 小区数量">
          <input
            className="h-9 w-full rounded-lg border border-[#ead8c2] bg-white px-3 text-sm text-[#33251f] outline-none focus:border-[#f3c99a]"
            type="number"
            min={1}
            ...
            onChange={(event) => setForm((prev) => ({ ...prev, top_communities: Number(event.target.value) || 1 }))}
            required
          />
        </Field>
      ...
      <div className="mt-4 space-y-3">
        {result?.work_location ? (
          <div className="rounded-lg border border-[#f3e1cb] bg-white/80 p-3 text-xs text-[#775f4d]">
            工作地点坐标：{result.work_location}
          </div>
        ) : null}

        {result?.summary ? (
          <div className="rounded-lg border border-[#f3e1cb] bg-white/80 p-3 text-xs text-[#775f4d]">
            候选房源：{String(result.summary.candidate_houses ?? 0)}，路线调用：{String(result.summary.route_calls ?? 0)}，小区 LLM
            重排：{String(result.summary.community_rerank_applied ?? false)}
          </div>
        ) : null}

        {!loading && result && communities.length === 0 ? (
          <div className="rounded-lg border border-[#f3e1cb] bg-white/80 p-3 text-sm text-[#775f4d]">
            当前条件下没有命中推荐小区，请调整预算、通勤或权重后重试。
          </div>
        ) : null}
...
function CommunityCard({
  ...
  const commuteLabel =
    community.median_commute_minutes === null ? "缺失" : `${Math.round(community.median_commute_minutes)} 分钟`;
  return (
    <Card className="border-[#f1dfc9] bg-[#fffdf8] shadow-none">
      ...
      <CardContent className="space-y-2 text-xs text-[#5f4a3d]">
        ...
        <div className="text-[11px] text-[#6e5543]">
          预算 {Math.round(community.budget_match_score * 100)} / POI {Math.round(community.poi_score * 100)} / 通勤{" "}
          {Math.round(community.traffic_score * 100)}
        </div>
          ...
          {houses.map((house) => (
            <div key={`${house.house_id}-${house.community_name}`} className="rounded-md border border-[#ecdcc6] bg-white p-2">
              ...
              <div className="mt-1 grid grid-cols-2 gap-1 text-[11px] text-[#6e5543]">
                ...
                <span>{house.total_price.toFixed(1)} 万元</span>
                <span>{house.commute_minutes === null ? "通勤缺失" : `通勤 ${Math.round(house.commute_minutes)} 分钟`}</span>
                <span>{house.area === null ? "面积缺失" : `${house.area.toFixed(1)} ㎡`}</span>
              </div>
            </div>
          ))}
        </div>
